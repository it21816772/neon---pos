...
export const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <button
      ...
      className="group relative flex min-h-[12rem] md:h-44 flex-col justify-between rounded-3xl border border-white/5 bg-white/5 p-4 text-left shadow-lg backdrop-blur-md transition hover:border-neon-cyan/60 hover:shadow-glow"
    >
      <div>
        <p className="text-xs uppercase tracking-[0.3em] text-white/50">{product.category?.name ?? 'General'}</p>
        <p className="mt-1 text-base md:text-lg font-semibold text-white group-hover:text-neon-cyan">{product.name}</p>
      </div>

      <div className="flex items-end justify-between text-sm text-white/70">
        <div>
          <p className="text-xl md:text-2xl font-semibold text-neon-cyan">{formatCurrency(product.priceCents)}</p>
